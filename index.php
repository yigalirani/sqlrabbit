...
function read_cookies(){
    fr_override_values($GLOBALS['settings'],unserialize(get_cookie('settings')));
}
function do_connect() {
    global $settings;
    global $conn;
...
function print_switch($table_class,$schema_class){
    $href = make_href2(null,array('database','table')).'&action';
    fr_placeholder_set('view_options',"(  <a $table_class href=$href=table>Data</a> | <a $schema_class href=$href=table_schema>Schema</a> )");
}
function databases_link() {
    return "<a href=index.php?action=databases>Databases</a>";
}
function database_link() {
    $database=fr_param('database');//todo: use deplating to reumse this line?
    return ("<a href=index.php?action=database&database=$database>$database</a>");
}
function print_sort_title($s) {
    ...
    if ($sort==$s){
        ...
        $href=make_href("dir",$other_dir,$nav_copy_fields);
        $img="<img src=$dir.png>";
        print("<td class=heading id=$s><a href=$href>$s  $img</a></td>\n");
    }else{
        $link=make_link($s,array('sort'=>$s,'dir'=>'asc'),$nav_copy_fields);
...
function print_cached_nav_result($query,$shown_columns,$first_column_decorator){
   ...
   print("</tr>");
   $start=intval(fr_int_param("start"));
...
function print_next_prev($dont_print_next,$start){
   ...
   if ($start >= $max_rows) {
       print(make_link('Last',array('start'=>$start - $max_rows),$nav_copy_fields));
   }else
       print "Last";
   print "&nbsp;&nbsp;&nbsp; |&nbsp;&nbsp;&nbsp";
   if (!$dont_print_next) {
       print(make_link('Next',array('start'=> $start + $max_rows),$nav_copy_fields));
   }else
       print "Next";
}
